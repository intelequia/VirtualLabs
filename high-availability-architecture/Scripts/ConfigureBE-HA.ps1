...
function SetupCB($compName, $clientURL)
{
	...
	try
	{
		Invoke-Command -ComputerName $compName -ScriptBlock { param($clientURL, $installPath, $domainNetbios)
            ...
            sleep -Seconds 10
			$result = (Start-Process -FilePath "msiexec.exe" -ArgumentList "/i ""$installPath"" /passive ADDLOCAL=ALL APPGUID={0CC618CE-F36A-415E-84B4-FB1BFF6967E1} IACCEPTMSODBCSQLLICENSETERMS=YES" -Wait -PassThru).ExitCode
...
Write-Output("Completed writing DNS Records")
 WriteLog("clientAccessName2: $($clientAccessNames)")
...
{
	WriteLog("Add-RdServer -Server $($localhost) -Role RDS-CONNECTION-BROKER -ConnectionBroker $($existingBroker)")
